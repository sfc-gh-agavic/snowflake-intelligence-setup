...
CREATE SCHEMA IF NOT EXISTS SNOWFLAKE_INTELLIGENCE.PUBLIC; -- Schema for public agents
CREATE SCHEMA IF NOT EXISTS SNOWFLAKE_INTELLIGENCE.SALES_INSIGHTS ;  -- Example schema sales_insights agents (adjust as needed)
...
GRANT CREATE AGENT ON SCHEMA SNOWFLAKE_INTELLIGENCE.SALES_INSIGHTS TO ROLE SALES_INSIGHTS_DEV_GROUP;  -- Only sales insights devs can create agents
...
GRANT USAGE ON AGENT SNOWFLAKE_INTELLIGENCE.SALES_INSIGHTS.<USE_CASE_AGENT> TO ROLE CREATE ROLE SALES_INSIGHTS_USER_GROUP;  -- Sales insights users can access a non-public agent
...
GRANT CREATE AGENT ON SCHEMA SNOWFLAKE_INTELLIGENCE.PUBLIC TO ROLE SNOWFLAKE_INTELLIGENCE_ADMIN;  -- Only admins can create public agents
...
GRANT ROLE SNOWFLAKE_INTELLIGENCE_ADMIN TO use <user1@yourcompany.com> ; -- Allocate out to admins (repeat as needed)
GRANT ROLE SALES_INSIGHTS_DEV_GROUP TO use <user2@yourcompany.com> ; -- Allocate out to devs (repeat as needed)
GRANT ROLE SALES_INSIGHTS_USER_GROUP TO use <user3@yourcompany.com> ; -- Allocate out to users (repeat as needed)
